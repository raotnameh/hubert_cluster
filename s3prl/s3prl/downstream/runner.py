...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        self.features_save_done = False # Have we saved the features or NO? 
        
        
        # if self.args.features_save_path == "no":
        #     self.args.features_save_path = False
        
      
        if self.args.mode == 'train' and self.args.features_save_path != 'no':
      
            self.args.features_save_path = f"{self.args.features_save_path}/{self.args.expname}"
            # self.args.features_save_path = f"{self.args.features_save_path}/{self.args.expname}"
            

            from os.path import exists
            file_exists = exists(self.args.features_save_path)
            if not file_exists:
                os.mkdir(self.args.features_save_path) 
            
            print(f"[Runner] - Features will be saved in this file: {self.args.features_save_path} for this task: {self.args.downstream}.")
        else: self.args.features_save_path = False
    ...
    def train(self):
        ...
        while pbar.n < pbar.total:
            ...
            if self.features_save_done:
                dummy = []
                names_k = [i.split(".")[0] for i in os.listdir(f"{self.args.features_save_path}")]
                for (_, *others) in dataloader:
                    for i in others[-1]:
                        dummy.append(i)

                if len(dummy) == len(names_k) and set(names_k) == set(dummy): 
                    self.features_save_done = True
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    if not self.features_save_done:
                        ...
                        if self.args.features_save_path:
                            for xoom, names_k in enumerate(others[-1]):
                                torch.save(features[xoom].to(torch.device('cpu')),f"{self.args.features_save_path}/{names_k}.pt")
                               
                    else: 
                        print("--------------")
                        features = [torch.load(f"{self.args.features_save_path}/{names_k}.pt",map_location="cuda") for names_k in others[-1]]
